		...
		if($ext == "csv"){
			...
			foreach($final_array as $f){
				...
				if(isset($f['part_number'])){
					$selectQ = "SELECT * FROM product_prices WHERE product_id = (SELECT id FROM product WHERE part_number = '".$f['part_number']."') AND user_id = (SELECT id FROM users WHERE company_name = (SELECT id FROM company_category WHERE category = '".$f['category']."'))";
					$founData = $conn->query($selectQ);
					if ($founData->num_rows > 0){
						while($rowData = $founData->fetch_assoc()) {
						...
						$insertsql = "INSERT INTO product_prices(id, product_id, user_id, product_price) VALUES (NULL, (SELECT id FROM product WHERE part_number = '".$f['part_number']."'), (SELECT id FROM users WHERE company_name = (SELECT id FROM company_category WHERE category = '".$f['category']."')), '".$f['price']."')";
						if ($conn->query($insertsql) === TRUE) {
							  echo "Data inserted successfully";
						}						
					}					
